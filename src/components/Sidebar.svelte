...
<header class="fixed top-0 left-0 right-0 z-50 transition-all duration-500 ease-soft {scrolled ? 'bg-cream-100/85 backdrop-blur-md border-b border-line' : 'bg-transparent border-b border-transparent'}">
  <nav class="max-w-6xl mx-auto px-6 sm:px-8">
    <div class="flex items-center justify-between h-16 md:h-20">
      <a href="/" class="flex items-center gap-3 group" on:click={closeMenu}>
        <img
          ...
          alt="Harry Chang"
          class="w-9 h-9 rounded-full object-cover ring-1 ring-line group-hover:ring-sienna-200 transition-all duration-300"
        />
        <span class="font-serif text-lg text-ink-400 hidden sm:block group-hover:text-sienna-400 transition-colors">
          Harry Chang
        </span>
      </a>
      ...
      <div class="hidden md:flex items-center gap-1">
        {#each navItems as item}
          ...
          >
            {$_(`nav.${item.key}`)}
            <span class="absolute left-3.5 right-3.5 -bottom-0.5 h-px bg-sienna-400 scale-x-0 group-hover:scale-x-100 transition-transform duration-300 ease-soft origin-left"></span>
          </a>
        {/each}
      </div>
    ...
    {#if isMenuOpen}
      <div class="md:hidden bg-cream-50 border-t border-line -mx-6 sm:-mx-8 px-6 sm:px-8 pb-6">
